...
除以$d_k$的目的是：
- 防止结果太大导致的梯度消失（与梯度爆炸无关）。如果QK相乘的结果过大，softmax会得到很大的概率，导致梯度消失；softmax对最大值敏感，这种偏移会导致softmax向one-hot趋近
- 假设Q、K是均值为0、方差为1的独立随机分布，QK相乘的方差为$d_k$。除以$\sqrt{d_k}$作用为标准化（回到均值0、方差1），避免softmax的输入分布偏移
