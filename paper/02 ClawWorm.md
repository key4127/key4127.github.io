...
- 验证配置文件完整性
- 减弱工具权限
- 供应链加固：这指的是加固clawhub上传到应用到每一个流程；比如为skill添加沙箱隔离、验证等
...
4. 对于同样的一套恶意skill配置流程，如果来自用户/agent，agent是否会区别对待？
5. 单纯的完整性验证会不会治标不治本？如果文件被感染，验证代码是否会感染（当然，验证代码感染的成本更高）

此外还有一个偏向产品的想法就是，对于这种“不确定”的ai应用，我们应该给用户赋予多高的权限，以及这种权限会不会和体验/宣传冲突。感觉需要一个高风险/开发者模式